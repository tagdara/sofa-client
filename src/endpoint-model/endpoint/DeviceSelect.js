...
import { categoryLabelByEndpointId, sortByName } from 'endpoint-model/discovery'
...
const DeviceSelect = props => {
    ...
    const devices = useDiscoveryStore( state => state.devices)
        ...
        ({ value, ...others }, ref) => (
            <div ref={ref} {...others}>
                <Group spacing={"xs"} noWrap style={{ padding: "2px 0px" }}>
                    <EndpointIcon size={16} endpointId={value} />
                    <Stack spacing={0} >
    ...
    return (
        <Select ref={selectRef}
                ...
                icon={props.icon ? <EndpointIcon size={16} endpointId={device} /> : undefined}
