...
export default function DeviceProvider(props) {
    ...
    function endpointIdsByCategory(category) {
        return Object.keys(devices).filter( endpointId => hasDisplayCategory(endpointId, category.toUpperCase()))
    }
    
    function endpointIdsByFriendlyName(names) {
        if (!Array.isArray(names)) { names=[names] }
        return Object.keys(devices).filter( endpointId => names.includes(devices[endpointId].friendlyName))
    }

    function hasDescription(endpointId, term) {
        return devices[endpointId].description.toLowerCase().includes(term.toLowerCase())
    }

    function friendlyNameByEndpointId(endpointId) {
        return devices[endpointId].friendlyName
    }
    ...
    return (
        <DeviceContext.Provider
            value={{
                ...
                endpointIdsByCategory: endpointIdsByCategory,
                endpointIdsByFriendlyName: endpointIdsByFriendlyName,
                friendlyNameByEndpointId: friendlyNameByEndpointId,
