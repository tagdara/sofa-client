...
export default function CameraSelect(props) {
    ...
    const { getEndpointIdsByCategory, devices, deviceState, unregisterDevices, directive } = useContext(DataContext);
    ...
    return (
        <React.Fragment>
            ...
            <SecurityCamera wide={props.wide} camera={currentCamera()} selectButtons={true} key={ userCamera } directive={ directive } deviceState={ deviceState(currentCamera()) }
                            name={ devices[currentCamera()].friendlyName } nextCamera={nextCamera} prevCamera={prevCamera} top={false} showOffline={true} />
            :null }
        </React.Fragment> 
    );
}